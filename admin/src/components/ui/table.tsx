...
export const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn("border-b border-white/8 transition hover:bg-white/[0.035] data-[state=selected]:bg-white/[0.035]", className)}
      {...props}
    />
  ),
);
...
export const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn("h-12 px-4 text-left align-middle text-[11px] font-semibold uppercase tracking-[0.2em] text-[var(--muted-foreground)]", className)}
      {...props}
    />
  ),
);
